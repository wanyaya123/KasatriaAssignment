...
import { fetchSheetData, getInitials, getNetWorthColor } from './data.js';
...
// Double helix — two intertwined spirals (like DNA), not the default single helix
function buildDoubleHelixTargets() {
	...
	const angleStep = (turns * Math.PI * 2) / itemsPerStrand;

	for (let i = 0, l = objects.length; i < l; i++) {
		const strand = i % 2; // alternate between inner and outer strand
		const indexOnStrand = Math.floor(i / 2);
		const radius = strand === 0 ? innerRadius : outerRadius;
		const theta = indexOnStrand * angleStep + strand * Math.PI; // offset second strand by half a turn
		const y = -(indexOnStrand * verticalStep) + ((itemsPerStrand - 1) * verticalStep) / 2;
		const object = new THREE.Object3D();
