...
// 4-face pyramid (tetrahedron) — 200 cards split as 50 on each triangular face
function buildTetrahedronTargets() {
	...
	const vertices = [
		...
		new THREE.Vector3(-1, 1, -1),
		new THREE.Vector3(-1, -1, 1),
	].map((vertex) => vertex.normalize().multiplyScalar(scale));
	...
	const perFace = Math.ceil(objects.length / faces.length);
	...
	for (let f = 0; f < faces.length; f++) {
		...
		const facePoints = sampleTriangleFace(a, b, c, perFace);

		for (let p = 0; p < facePoints.length; p++) {
			const index = f * perFace + p;
			if (index >= objects.length) break;

			const object = new THREE.Object3D();
			object.position.copy(facePoints[p]).addScaledVector(normal, 20);
			faceCardOutwardStraight(object, normal);
			targets.tetrahedron.push(object);
		}
	}
}

// Fills a triangle like bowling pins: 1, then 2, then 3... so 50 cards sit in even rows
function sampleTriangleFace(a, b, c, count) {
	const rowSizes = [];
	let size = 1;
	let left = count;
	...
	for (let row = 0; row < rowCount; row++) {
		...
		const v = (row + 1) / (rowCount + 1);
		const leftPoint = a.clone().lerp(b, v);
		const rightPoint = a.clone().lerp(c, v);

		for (let col = 0; col < cols; col++) {
			const u = cols === 1 ? 0.5 : (col + 1) / (cols + 1);
			points.push(leftPoint.clone().lerp(rightPoint, u));
		}
	}
...
// Face the card outward from the pyramid, but keep the text upright (not rolled sideways)
function faceCardOutwardStraight(object, normal) {
	const worldUp = new THREE.Vector3(0, 1, 0);
	const zAxis = normal.clone().normalize();
	let yAxis = worldUp.clone().sub(zAxis.clone().multiplyScalar(worldUp.dot(zAxis)));
